...
def run_inference(
    ...
    # Open CSV in append mode — avoids rewriting the full file on every image.
    # If resuming, the header was already written on the previous run, so skip it.
    write_header = not (resume and out_csv.exists())
    csv_file = open(out_csv, "a", newline="", encoding="utf-8")

    try:
        for img_path in tqdm(to_process, desc="Tagging"):
            record = infer_image(
                client, img_path,
            ...
            # Write this single row immediately — crash-safe, O(1) per image
            row_df = pd.DataFrame([record])[cols]
            row_df.to_csv(csv_file, index=False, header=write_header)
            csv_file.flush()
